...
class Migration1783300000MapJtlOrderCustomFields extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
         ...
         * Backfill the fixed keys from the existing camelCase values for every order that already
         * has the mollie_payments object.
         */
        $sql = <<<'SQL'
            UPDATE `order`
            SET `custom_fields` = JSON_SET(
                ...
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(`custom_fields`, '$.mollie_payments.orderId')), ''),
                '$.mollie_payments.payment_id',
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(`custom_fields`, '$.mollie_payments.id')), ''),
                '$.mollie_payments.third_party_payment_id',
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(`custom_fields`, '$.mollie_payments.thirdPartyPaymentId')), '')
            )
