 ...
 * @internal
 */
final class Migration1778100100SubscriptionPriceChangeMailTemplate extends MigrationStep
{
    ...
    {
        $englishName = 'Subscription Price Change Notice (Mollie)';
        $germanName = 'Preisänderung des Abonnements (Mollie)';
    ...
    private function ensureMailTemplate(Connection $connection, string $mailTemplateTypeId): void
    {
        ...
        $en = [
            'subject' => 'The price of your subscription with {{ salesChannel.name }} will change',
            'description' => 'Subscription Price Change Notice Mail',
            'contentHtml' => $this->loadTemplateFile('en.html'),
            'contentPlain' => $this->loadTemplateFile('en.txt'),
        ];

        $de = [
            'subject' => 'Der Preis Ihres Abonnements bei {{ salesChannel.name }} ändert sich',
            'description' => 'Benachrichtigungsmail zur Preisänderung des Abonnements',
