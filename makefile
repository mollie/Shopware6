...
ifneq (,$(findstring v12,$(NODE_VERSION)))
	...
	cd .. && zip -qq -r -0 ./.build/MolliePayments.zip MolliePayments/[^.]* -x '*/vendor/*'  '*.git*' '*.reports*' '*/.idea*' '*/tests*' '*/node_modules*' '*/makefile' '*.DS_Store' 'config/*' '.prettierignore' './package.json' './package-lock.json'
	cd .. && zip -qq -r -0 ./.build/MolliePayments-e2e.zip MolliePayments/[^.]* -x '*/vendor/*'  '*.git*' '*.reports*' '*/.idea*' '*/node_modules*' '*.DS_Store' '.prettierignore' './package.json' './package-lock.json'
	# -------------------------------------------------------------------------------------------------
	# -------------------------------------------------------------------------------------------------
	@echo ""
